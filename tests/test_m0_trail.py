...
本文件里体现的设计决定（请确认）：
1. ``mark()`` 返回 trail 位置（int），``rollback(mark)`` 撤销到该位置；向前撤销抛 ``ValueError``。
2. ``bindings()`` 只列出非代表元变量，值为其 ``resolve`` 结果（可能是另一个变量，此时是 id 较小的代表元）。
"""
import pytest
...
class TestMarkUndo:
    ...
    def test_rollback_current_mark_is_noop(self):
        st = State()
        x = fresh_var()
        assert st.unify(x, atom("NP"))
        m = st.mark()
...
class TestDump:
    ...
    def test_bindings_var_var_shows_representative(self):
        ...
        x, y = fresh_var(), fresh_var()
        assert st.unify(x, y)
        assert st.bindings() == {y: x}  # 较大 id 指向较小 id
