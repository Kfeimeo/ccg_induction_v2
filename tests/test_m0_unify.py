...
4. 合并方向确定：两个自由变量合并时代表元永远是 id 较小者（不按秩），golden 输出不随操作顺序漂移。
"""
...
class TestBasic:
    ...
    def test_var_var_then_bind(self):
        ...
        assert st.unify(x, y)
        assert st.same(x, y)
        assert st.resolve(x) is x and st.resolve(y) is x  # 代表元 = 较小 id
        assert st.unify(y, atom("NP"))
        assert st.resolve(x) is atom("NP")

    def test_merge_direction_is_order_independent(self):
        a, b, c = fresh_var(), fresh_var(), fresh_var()
        st1 = State()
        assert st1.unify(c, b) and st1.unify(b, a)
        st2 = State()
        assert st2.unify(a, b) and st2.unify(c, a)
        for st in (st1, st2):
            assert st.resolve(a) is a and st.resolve(b) is a and st.resolve(c) is a
        assert st1.bindings() == st2.bindings() == {b: a, c: a}
